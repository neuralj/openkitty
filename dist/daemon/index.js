...
async function main() {
    ...
    let proc;
    const cooldown = new CooldownManager(client, { pause: () => proc.pause(), resume: () => proc.resume() }, cfg.pingIntervalMs, cfg.model);
    ...
    if (mode === "submit") {
        ...
        for (;;) {
            ...
            if (!proc.isPaused())
                await proc.processOne();
            await sleep(1000);
        }
        process.exit(0);
    }
    // 常驻 daemon 模式
    console.log(`[daemon] openhub-daemon server=${cfg.serverUrl} dirs=${cfg.directories} ping=${cfg.pingIntervalMs}ms db=${cfg.dbPath}`);
