...
export class QueueProcessor {
    ...
    defaultModel;
    paused = false;
    constructor(client, store, cooldown, events, defaultModel = "") {
        this.client = client;
        this.store = store;
    ...
    async processOne() {
        ...
        if (!task)
            return false;
        if (task.agentID && this.cooldown.has(task.agentID))
            return false;
        ...
        try {
            ...
            const { aborted } = await this.client.sendMessage(agentID, task.prompt, task.model || this.defaultModel);
            if (aborted) {
    ...
    async recoverStaleRunning() {
        ...
        for (const t of all) {
            ...
            try {
                const msgs = (await this.client.getMessages(t.agentID));
