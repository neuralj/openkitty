// PipelineRunner: 顺序流水线编排（L1 原子能力）
// 不改动 QueueProcessor，通过监听 task 事件驱动阶段推进
// 每个 stage 提交为普通任务，pipeline 是 task 之上的编排层
import type { Store, TaskRecord, PipelineRecord } from "./store.js";
...
export class PipelineRunner {
  ...
  async start(pipelineId: string): Promise<PipelineRecord> {
    const pl = await this.store.getPipeline(pipelineId);
    if (!pl) throw new Error(`pipeline ${pipelineId} not found`);
    if (pl.stages.length === 0) throw new Error("pipeline has no stages");
  ...
  /** 提交一个 stage 作为普通任务 */
  private async submitStage(pl: PipelineRecord, stageIndex: number): Promise<void> {
  ...
  async onTaskCompleted(task: TaskRecord): Promise<void> {
    ...
    if (!current) return;
    if (current.status === "pending") return; // 自动重试中，不推进 pipeline
    if (current.status === "running") return;  // 仍在执行
  ...
  init(): void {
    ...
    this.events.on("task", (p: unknown) => {
      const payload = p as { id?: string; status?: string };
      if (
        payload.id &&
        (payload.status === "completed" || payload.status === "failed")
      ) {
        this.store.get(payload.id).then((task) => {
          if (task?.pipelineId) this.onTaskCompleted(task);
        });
      }
    });
